...
3. 将batch内的序列打包成一个样本，序列并行计算。无pading，不同样本直接长度差异大。
<img src="docs/image-3.png">
4. 按指定最大长度组织数据，打包直到快超出最大长度。无pading，不同样本直接长度差异微乎其微。例如指定128K，在distill_r1_110k数据集上可以压缩至1k样本量。
<img src="docs/image-4.png">

OpenSFT为实现第4种数据组织方式，采用离线计算的方式，将训练和数据处理进行解耦。
